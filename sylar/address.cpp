...
// 获取当前系统的所有网络接口(网卡)及其对应的 IP 地址和子网掩码前缀长度, 并将结果存入 result 参数中
bool Address::GetInterfaceAddresses(std::multimap<std::string
    ...
    if (getifaddrs(&results) != 0) {
        std::cout << "Address::GetInterfaceAddress getifaddrs "
    ...
    try {
        ...
        for (next = results; next; next = next->ifa_next) {
            ...
            // family 不是任意地址族 且 与当前的地址族不匹配, 过滤
        ...
        std::cout << "Address::GetInterfaceAddresses exception";
        freeifaddrs(results);
        return false;
    }
    // 释放内存?
