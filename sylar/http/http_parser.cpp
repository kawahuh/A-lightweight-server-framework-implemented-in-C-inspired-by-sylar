...
void on_request_method(void* data, const char* at, size_t length) {
    ...
    if (m == HttpMethod::INVALID_METHOD) {
        std::cout << "invalid http request method: "
...
void on_request_version(void* data, const char* at, size_t length) {
    ...
    if (strncmp(at, "HTTP/1.1", length) == 0) {
        ...
        std::cout << "invalid http request version " << std::string(at, length) << std::endl;
...
void on_request_http_field(void* data, const char* field, 
    ...
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
    if (flen == 0) {
        std::cout << "invalid http request field length == 0" << std::endl;
        return ;
    }
    parser->getData()->setHeader(std::string(field, flen)
                                ,std::string(value, vlen));
